<template>
  <div class="menu">
    <div class="menulist-title menu-item">
      ...
      <Badge v-if="badgeCount"
             class="badge-count"
             :count="badgeCount"
             overflow-count="9999"></Badge>
    </div>
    <div>
      <ul v-for="(item, index) in data">

        <li class="menu-item menu-folder"
            :id="item.folderId"
            @click="openMenu(item, index,item.folderId)"
            @contextmenu.prevent="show(item)">
          <span class="list-title">{{ item.folderName }}</span>
          <i class="iconfont"
             :class="{'iconfont-hidden':!item.children.length}"
             v-if="item.children">&#xe63b;</i>
          <Badge :count="item.targetCount"
                 class="badge"
                 overflow-count="9999"></Badge>
        </li>
        ...
        <li class="menu-item menu-item-children"
            ...
            :id="childItem.folderId"
            :class="{'menu-active':activeChildIndex === childIndex && activeIndex === index}"
