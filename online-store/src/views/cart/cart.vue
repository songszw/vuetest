<template>
  <div id="main">
    <div class="top-next cle">
      <div class="fr"> <a class="graybtn"
           @click="continueShopping">继续购物</a>
        <a class="btn"
           id="checkout-top"
           @click="balanceCount">&nbsp;去结算&nbsp;</a> </div>
    </div>
    <div class="cart-box"
         id="cart-box">
      <div class="hd"> <span class="no2"
              id="itemsnum-top">{{goods.goods_list.length}}件商品</span>
      ...
      <div class="goods-list">
        <ul>
          <li class="cle hover"
              ...
              v-for="(item,index) in goods.goods_list">
            <div class="pic">
              <a target="_blank"> <img :alt="item.goods.name"
                     :src="item.goods.goods_front_image"></a>
            </div>
            <div class="name">
            ...
            <div class="nums"
              ...
              <span class="minus"
                    title="减少1个数量"
                    @click="reduceCartNum(index, item.goods.id);">-</span>
              <input type="text"
                     v-model="item.nums">
              <span class="add"
                    title="增加1个数量"
                    @click="addCartNum(index, item.goods.id);">+</span>
            </div>
            ...
            <div class="del">
              <a class="btn-del"
                 @click="deleteGoods(index, item.goods.id)">删除</a>
            </div>
          </li>
        </ul>
      </div>
      ...
      <div class="fd cle">
        <div class="fl">
          <p class="no1"> <a id="del-all"
               @click="delAll">清空购物车</a> </p>
          <p><a class="graybtn"
               @click="continueShopping">继续购物</a></p>
        </div>
        <div class="fr"
             id="price-total">
        ...
        <div class="extr">
          <div class="address">
            ...
            <ul>
              <li class="add"
                  @click="addAddr">
                <router-link :to="'/app/home/member/receive'"
                             target=_blank>
                  +
                  点击添加地址</router-link>
              </li>
              <li v-for="item in addrInfo"
                  :class="{'addressActive':addressActive==item.id}"
          ...
          <div class="pay">
            ...
            <p class="payWrap"><img v-for="item in payWrapList"
                   src="../../static/images/alipay.jpg"
                   :class="{'payWrapActive':payWrapActive==item.id}"
                   @click="selectPay(item.id)"></p>
          </div>
        </div>
        <textarea type="text"
                  v-model="post_script"
        ...
        <p class="sumup"><a class="btn"
             @click="balanceCount">去结算</a></p>
      </div>
    </div>
  </div>
